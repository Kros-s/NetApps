...
int main(int argc, char const *argv[])
{
	...
	if((bind(sock,(const struct sockaddr*)&direccion,sizeof(struct sockaddr_in)))== -1)
        handle_error("Bind");
	...
	while(1){
		int x=7;
		newsock = accept(sock, (struct sockaddr *) &peer_addr,&peer_addr_size);// AQUI VA EL FORK 
		if (newsock == -1)
	        handle_error("accept");
	    else{
	    	//CREO Q ES MEJOR CON UN IF 
			if(fork()==0){
				/*Codigo del hijo*/
				int buffsiz;			
				if((buffsiz =recv(newsock, buffer, sizeof(buffer),0))==-1)
					handle_error("read");				
